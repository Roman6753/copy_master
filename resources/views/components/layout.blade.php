...
<ul class="flex gap-x-4">
    ...
    @auth
        ...
        @if(auth()->user()->isAdmin())
            <li><x-link href="{{ route('admin.categories') }}">Админ-панель</x-link></li>
        @endif
        <li>
            <form action="{{ route('logout') }}" method="POST">
                @csrf
                <button type="submit" class="uppercase text-sm font-semibold text-gray-500 hover:text-gray-800">
                    Выход
                </button>
            </form>
        </li>
